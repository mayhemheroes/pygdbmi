#!/usr/bin/env bash
#
# mayhem/test.sh — RUN pygdbmi's own pytest suite (deps already installed by mayhem/build.sh) and
...
# the pygdbmi_run_tests ELF wrapper. We only RUN the suite. The suite is routed through that
...
#
# Scope: the PURE-PYTHON test files (the GDB/MI parser, the escape handling, the string stream) —
# exactly what the fuzz harness exercises. tests/test_gdbcontroller.py and tests/test_example.py
# drive a live `gdb` subprocess and build a C sample app (make), neither of which is present in the
# commit image, so they are intentionally excluded from the oracle.
set -uo pipefail
...
# Put the in-image site dir (atheris + pytest) + pygdbmi source on PYTHONPATH.
...
# Run the pure-Python suite. -p no:cacheprovider keeps the read-only image happy.
LOG="$(mktemp)"
"$RUNNER" -p no:cacheprovider -o addopts= -q \
    tests/test_gdbmiparser.py tests/test_gdbescapes.py tests/test_string_stream.py 2>&1 | tee "$LOG"
rc=${PIPESTATUS[0]}
