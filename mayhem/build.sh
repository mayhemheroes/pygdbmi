#!/usr/bin/env bash
#
# mayhem/build.sh — build the pygdbmi Atheris fuzz harness + its standalone reproducer,
...
#      resolves entirely from it (pip --no-index --find-links). pygdbmi itself stays the editable
#      source tree (added to PYTHONPATH) so a PATCH agent's edits under pygdbmi/ take effect.
#   2. Compile launcher.c -> the ELF Mayhem target `pygdbmi_fuzzer` (Atheris is a Python script;
#      Mayhem needs an ELF cmd, and the gate needs DWARF < 4 — hence a compiled wrapper).
#   3. Build the same launcher as the standalone (run-once) reproducer `pygdbmi_fuzzer-standalone`.
...
#    manylinux wheel for this CPython; pytest is the suite runner. pygdbmi is pure-Python and stays
#    the editable source tree (no install needed).
PKGS=(atheris pytest)
need_download=0
...
# pygdbmi itself: flat layout (the pygdbmi/ package sits at the repo root), so expose it by putting
# $SRC on PYTHONPATH — both via the baked ENV in the Dockerfile (run time) and via env.sh / the
# explicit sanity check below (build time).
PYRUN="$SITE:$SRC"
...
PYTHONPATH="$PYRUN" "$PY" -c 'import atheris, pytest; from pygdbmi import gdbmiparser; print("imports OK")'
...
#    at run time (the Dockerfile sets ENV PYTHONPATH), so the Python side finds atheris + pygdbmi.
HARNESS="$SRC/mayhem/fuzz_gdb.py"
echo ">> compiling pygdbmi_fuzzer (+ standalone) with DEBUG_FLAGS=$DEBUG_FLAGS"
$CC $DEBUG_FLAGS -DPYTHON="\"$PY\"" -DHARNESS="\"$HARNESS\"" \
    "$SRC/mayhem/launcher.c" -o "$SRC/pygdbmi_fuzzer"
...
$CC $DEBUG_FLAGS -DPYTHON="\"$PY\"" -DHARNESS="\"$HARNESS\"" \
    "$SRC/mayhem/launcher.c" -o "$SRC/pygdbmi_fuzzer-standalone"
...
$CC $DEBUG_FLAGS -DPYTHON="\"$PY\"" "$SRC/mayhem/run_tests.c" -o "$SRC/pygdbmi_run_tests"

echo ">> build.sh complete"
ls -la "$SRC/pygdbmi_fuzzer" "$SRC/pygdbmi_fuzzer-standalone" "$SRC/pygdbmi_run_tests"
